...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
      ...
      <html>
         <head>
            ...
            <style> 
               ...
               font-size: 1em; margin: 25px; width: 800px; } 
               ...
               .revTable {font-size: 1em; font-family: Verdana, Arial, Helvetica, sans-serif; }
               
               .links {font-size: small; text-align: center; } 
               
               .bibl { text-indent: -1em; margin-left: 1em; }
               
               .float_right { float: right; margin: 10px 10px 10px 10px; } 
         ...
         <body>
            ...
            <p class="projectTitle">Edited by <xsl:value-of select="//titleStmt/editor"/></p>
            <hr/>
            <p class="links">[ <a href="#source">About the Source Document</a> | <a
               href="#edition">About the Electronic Edition</a> | <a href="#revision">Revision
                  History</a> | 
               <xsl:element name="a">
                  <xsl:attribute name="href">http://suse1.cohums.ohio-state.edu/tomcat/cocoon/eng569/<xsl:value-of select="//TEI.2/@id"/>.WBA_reading</xsl:attribute>Text of the Edition</xsl:element> ]
            </p>
            <hr/>
            ...
            <p>Link to the 
               <xsl:element name="a">
                  <xsl:attribute name="href">http://suse1.cohums.ohio-state.edu/tomcat/cocoon/eng569/<xsl:value-of select="//TEI.2/@id"/>.WBA_reading</xsl:attribute>Text of the Edition
               </xsl:element>
            </p>
         </body>
      </html>
   ...
   <xsl:template match="biblFull">
         <hr/>
         <!-- Add an image to your Introduction by un-commenting the following line,
         placing the filename.ext in the "src" attribute, and entering width and height
         dimensions that correspond to the size of your image.
         
         <img src="" alt="Media" name="Media" width="250" height="250" class="float_right" id="Media" />

         -->
         <a name="source"/>
         <h2>About the Source Document</h2>
         <p>Title: "<xsl:value-of select="titleStmt/title"/>" 
            <br/>Author: <xsl:value-of select="titleStmt/author"/>
            <br/>Extent: <xsl:value-of select="extent"/>
         </p>
      <p>
         <xsl:apply-templates select="notesStmt/note"/>
      </p>
      <p>To view a zoomable image of the first page of this manuscript, 
      <a>
         <xsl:attribute name="href">http://people.cohums.ohio-state.edu/ulman1/WBAnderson_Letters/AndersonZoom.cfm?file=<xsl:value-of select="//pb[@n=1]/@mmid"/>.jpg</xsl:attribute>
         <xsl:attribute name="title">Link to zoomable image of manuscript page.</xsl:attribute>
         click here.
      </a></p>
      <p>
         <xsl:apply-templates select="fileDesc/publicationStmt"/>
   ...
   <xsl:template match="refsDecl">
      <h3>Encoding and Naming Conventions</h3>
      <xsl:apply-templates/>
   </xsl:template>
   ...
   <xsl:template match="titleStmt">
         <hr/>
         <a name="edition"/><h2>About the Electronic Edition</h2>
         <p>Title: <xsl:value-of select="title"/>
         <br/>Editor: <xsl:value-of select="editor"/>
   ...
   <xsl:template match="editorialDecl">
      <h3>Editorial Method and Apparatus</h3>
      <xsl:apply-templates/>
   </xsl:template>
   ...
   <xsl:template match="revisionDesc">
      <p>
         ...
         <hr/>
         <a name="revision"/><h2>Revision History</h2>
      </p>
      <table width="90%">
         <tr>
            <td>
