...
<hr/>
            <p align="center"><a href="http://people.cohums.ohio-state.edu/ulman1/WBAnderson_Letters/" target="self">
               ...
               <strong>Reading (</strong> <a href="#view">About the current view</a>) |
               <A>
                  <xsl:attribute name="HREF">http://suse1.cohums.ohio-state.edu/tomcat/cocoon/eng569/<xsl:value-of select="TEI.2/@id"/>.WBA_diplomatic</xsl:attribute>
                  <xsl:attribute name="target">self</xsl:attribute>
                  Diplomatic</A> |
...
<hr/><p><strong>About this View of the Letters</strong></p>
            <p><a name="view"/>In the text of the letter, line breaks in the manuscript are not reported; rather
               lines wrap in your browser's window. Page breaks are not reported. Paragraph breaks follow those in the 
               ...
               Text canceled by Anderson is suppressed in this view. Text added by Anderson between 
               lines or in the margins is silently incorporated into the text. All material added by the editor is 
               surrounded by square brackets: uncertain readings 
               are enclosed in square brackets and followed by a question mark; gaps in the transcription and
               Anderson's deletions are noted by ellipses within square brackets; and text supplied by the 
               editor for clarity are set in italics and surrounded by 
               square brackets. Links to notes and external materials are colored and underlined. Finally, the 
...
<hr/>
   ...
   <xsl:template match="seg[@type='softhyphen']"></xsl:template>
...
<xsl:template match="opener">
   ...
   <xsl:template match="salute"><br/><xsl:apply-templates/></xsl:template>
